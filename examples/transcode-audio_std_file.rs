...
fn main() {
	...
	let input_path = env::args().nth(1).expect("missing input");
	let output_path = env::args().nth(2).expect("missing output");
	...
	let mut ictx = format::input_io(input).unwrap();
	let mut octx = format::output_named_io(output, &output_path).unwrap();
